// prompt-rewriter/app/api/labs/pipeline/strategies/[ns]/route.ts
//
// 策略 namespace 列表与新建。
//   GET  /api/labs/pipeline/strategies/<ns>           → 列所有版本(_index.json)
//   POST /api/labs/pipeline/strategies/<ns>           → 新建版本(publish)
//
// URL 的 <ns> 直接是 registry namespace（"vertical-standard" / "platform-tone"）。
...
function checkStrategyNs(
  raw: string,
): { ok: true; ns: Namespace } | { ok: false; res: NextResponse } {
  if (!isNamespace(raw) || !ALLOWED_STRATEGY_NS.includes(raw as never)) {
    return {
      ok: false,
      res: NextResponse.json(
        { error: `unknown strategy namespace: ${raw}` },
        { status: 400 },
      ),
    };
  }
...
export async function GET(
  ...
  const check = checkStrategyNs(rawNs);
